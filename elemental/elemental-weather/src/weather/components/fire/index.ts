...
export default class Fire extends Object3DComponent {
    ...
    private async createFireMaterial(): Promise<void> {
        const fireTexture = resourcesManager.getItemById("fireTexture");
        if (fireTexture) {
    ...
    private async createSmokeMaterial(): Promise<void> {
        const smokeTexture = resourcesManager.getItemById("smokeTexture");
        if (smokeTexture) {
    ...
    private async createAmberMaterial(): Promise<void> {
        const amberTexture = resourcesManager.getItemById("particleTexture");
        if (amberTexture) {
    ...
    private createFireEmitter(): void {
        ...
        const fireShape = fireEmitterParams.shape as PointShape;
    ...
    private createSmokeEmitter(): void {
        ...
        const smokeShape = smokeEmitterParams.shape as PointShape;
    ...
    private createAmberEmitter(): void {
        const amberEmitterParams = new EmitterParams();
        amberEmitterParams.shape = new PointShape();
        const amberShape = amberEmitterParams.shape as PointShape;
        ...
        const amberRendererParams = new ParticleRendererParams();
        amberRendererParams.maxParticles = amberEmitterParams.maxParticles;
        amberRendererParams.group = new Three.Group();

        amberEmitterParams.renderer = new ParticleRenderer();
        amberEmitterParams.renderer.initialize(
            this.amberMaterial!,
            amberRendererParams
        );

        const amberEmitter = new Emitter(amberEmitterParams);
