...
export default class Fog extends Object3DComponent {
    ...
    constructor(scene: SceneWrapper, options: {
        ...
        super(scene, 'weather-fog', options.isDebugMode);
    ...
    private createFogColorPresets(): FogColorPresets {
        return {
            ...
            rainy: {
                ...
                night: new Three.Color(0.004, 0.012, 0.020),
            },
        };
    }
    ...
    private getFogColor(): Three.Color {
        ...
        const seasonColors = this.fogColors[currentSeason];
        if (!seasonColors) {
            this.logger.warn(`[Fog] No color preset for season: ${currentSeason}, using spring`);
            return this.fogColors['spring'].day;
        }
        const day = datetimeManager.isDaytime() ? 'day' : 'night';
        return seasonColors[day] || seasonColors.day;
    }
