...
import {GrassManager} from "/@/utils/GrassManager.ts";
...
export default class Ground {
    constructor({
        ...
        this.biomeManager = new BiomeManager(this.weather, this.WORLD_SIZE);
    ...
    setGrid() {
        ...
        const biomeTexture = this.weather.resources.items.grassPathDensityDataTexture;
        biomeTexture.wrapS = biomeTexture.wrapT = THREE.ClampToEdgeWrapping;

        const displacementTexture = this.weather.resources.items.displacementMap;
        displacementTexture.wrapS = displacementTexture.wrapT =
            THREE.RepeatWrapping;
        const perlinNoise = this.weather.resources.items.perlinNoise;
        perlinNoise.wrapS = perlinNoise.wrapT = THREE.RepeatWrapping;

        const groundRockMap = this.weather.resources.items.groundRockMap;
        groundRockMap.wrapS = groundRockMap.wrapT = THREE.RepeatWrapping;

        const groundRockAO = this.weather.resources.items.groundRockAOMap;
        ...
        const configureTexture = (texture, repeat = 1) => {
            ...
            texture.anisotropy =
                this.weather.renderer.rendererInstance.capabilities.getMaxAnisotropy();
    ...
    addWaterRipples() {
        ...
        const waterDepthTexture = this.weather.resources.items.waterDepthMap;
