...
export default class Lightning {
    ...
    createExplosionMaterial() {
        const particleTexture = this.weather.resources.items.particleTexture;
        if (particleTexture) {
    ...
    createArcMesh(position) {
        ...
        const startTime = this.weather.time?.elapsedTime ?? performance.now() / 1000;
    ...
    triggerCameraShake(strikePosition = null) {
        const camera = this.weather.camera.cameraInstance;
    ...
    strike(position) {
        ...
        if (this.weather.ambientSoundManager) {
            this.weather.ambientSoundManager.playThunderStrike();
        }
    ...
    updateBoundsForAspectRatio() {
        const sizes = this.weather.sizes;
    ...
    isRainySeason() {
        return this.weather.seasonManager?.currentSeason === 'rainy';
    }

    update(delta) {
        this.elapsedTime += delta;

        const currentTime = this.weather.time?.elapsedTime || performance.now() / 1000;
    ...
    initGUI() {
        if (!this.weather.debug) return;

        const folder = 'Lightning/Explosion Particles';

        this.weather.debug.add(
            this.explosionParticles,
            'count',
